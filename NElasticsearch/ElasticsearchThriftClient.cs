...
{
	public class ThriftConnection : ElasticsearchClient, IDisposable
	{
        private readonly ClientsPool<Rest.Client> _clients = new ClientsPool<Rest.Client>();
...
//			new ConcurrentDictionary<Uri, ConcurrentQueue<Rest.Client>>();
		...
		{
			var restRequest = new RestRequest();
            restRequest.Method = RestMethod.HEAD;
			restRequest.Uri = uri;
		...
		private ElasticsearchResponse<Stream> Execute(RestRequest restRequest, object requestConfiguration)
		{
			...
			//
			var method = Enum.GetName(typeof(RestMethod), restRequest.Method);
